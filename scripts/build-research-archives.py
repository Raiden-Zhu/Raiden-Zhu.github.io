...
def row(p,n):
 ...
 badge = '<span class="venue-badge venue-badge--split"><span class="venue-conference">ICLR 2026</span><span class="venue-honor"><strong>Oral</strong> · Top 1.2%</span></span>' if p["id"] == "zhu2026surprising" else f'<span class="venue-badge">{esc(p["abbr"])}</span>'
 ...
 return f'''<article class="paper" id="paper-{n}"><a class="paper-visual" href="{esc(primary)}" aria-label="Read {esc(p['title'])}">{badge}<img src="/assets/img/publication_preview/{p['image']}" alt="{esc(p['summary'])}" width="1672" height="941" loading="lazy"></a><div class="paper-body"><p class="paper-kicker"><span>{n:02d}</span><span>{esc(p['abbr'])}</span></p><h3>{link(primary,p['title'])}</h3><p class="takeaway">{esc(p['summary'])}</p><p class="authors">{authors}</p>{links(p['links'])}</div></article>'''
